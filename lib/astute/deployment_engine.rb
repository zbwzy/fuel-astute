...
module Astute
  class DeploymentEngine
    ...
    def deploy(deployment_info, pre_deployment=[], post_deployment=[])
      ...
      Astute.logger.info "************** deployment_engine.rb method deploy begin ***********************"
      ...
      deployment_info.sort_by { |f| f['priority'] }.group_by{ |f| f['priority'] }.each do |_, nodes|
        # Astute.logger.info "============== nodes: #{nodes} =============="
        ...
        group_by_uniq_values(nodes).each do |nodes_group|
          ...
          nodes_group.each_slice(Astute.config[:MAX_NODES_PER_CALL]) do |part|
            if !fail_deploy
	      # Astute.logger.info "============== part: #{part} =============="
              # Pre deploy hooks
              # pre_node_actions.process(part)
	      # Astute.logger.info "************** deployment_engine.rb method deploy pre_node_actions end ***********************"

              PreDeployActions.new(part, @ctx).process
              Astute.logger.info "************** deployment_engine.rb method deploy PreDeployActions end ***********************"

              deploy_piece(part)
	      Astute.logger.info "************** deployment_engine.rb method deploy deploy_piece end ***********************"

              # Post deploy hook
              # PostDeployActions.new(part, @ctx).process
	      # Astute.logger.info "************** deployment_engine.rb method deploy PostDeployActions end ***********************"
      ...
      Astute.logger.info "************** deployment_engine.rb method deploy Post deployment hooks end ***********************"
