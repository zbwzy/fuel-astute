...
module Astute
  module PuppetdDeployer
    ...
    def self.deploy(ctx, nodes, retries=2, puppet_manifest=nil, puppet_modules=nil, cwd=nil)
      Astute.logger.info "************** puppetd.rb method deploy ***********************"
      @ctx = ctx
    ...
    def self.puppetd_runonce(uids)
      Astute.logger.info "************** puppetd.rb method puppetd_runonce ***********************"
	...
	Astute.logger.info "============== uids: #{uids} =============="
	Astute.logger.info "============== running_uids: #{running_uids} =============="
    ...
    def self.calc_nodes_status(last_run, prev_run, hung_nodes=[])
      ...
      Astute.logger.info "************** puppetd.rb method calc_nodes_status ***********************"

      hung_nodes = hung_nodes & last_run.map { |n| n.results[:sender] }
      Astute.logger.info "============== hung_nodes: #{hung_nodes} =============="

      stopped = last_run.select { |x| ['stopped', 'disabled'].include? x.results[:data][:status] }
      Astute.logger.info "============== stopped: #{stopped} =============="
      ...
      Astute.logger.info "============== succeed_nodes: #{succeed_nodes} = error_nodes: #{error_nodes} = running_nodes: #{running_nodes} ============"
    ...
    def self.deploy_nodes(nodes_to_check)
      Astute.logger.info "************** puppetd.rb method deploy_nodes begin **************nodes_to_check: #{nodes_to_check}*********"
      Timeout::timeout(Astute.config.PUPPET_TIMEOUT) do
        prev_summary = puppetd(nodes_to_check).last_run_summary
	Astute.logger.info "************** puppetd.rb last_run_summary end**********prev_summary: #{prev_summary}*************"
        hung_nodes = puppetd_runonce(nodes_to_check)
	sleep Astute.config.PUPPET_DEPLOY_INTERVAL * 2
	Astute.logger.info "************** puppetd.rb puppetd_runonce end**********node_retries: #{@node_retries}*************"
	Astute.logger.info "************** puppetd.rb puppetd_runonce end**********hung_nodes: #{hung_nodes}*************"

	while nodes_to_check.present?
	  Astute.logger.info "============== nodes_to_check begin: #{nodes_to_check} =============="

          last_run = puppetd(nodes_to_check).last_run_summary
	  Astute.logger.info "============== last_run: #{last_run} =============="
          Astute.logger.info "============== prev_summary: #{prev_summary} =============="
          #Astute.logger.info "============== hung_nodes: #{hung_nodes} =============="
	  ...
	  Astute.logger.info "============== nodes_to_check end: #{nodes_to_check} =============="
